#!/usr/bin/env python3
"""
政治ニュース自動収集Bot（国内＆トランプ氏特化・AIコメント付き重複防止版）
"""
...
# 政治関連キーワード（※他国の首脳や国名を削除し、日米とトランプ氏に絞りました）
POLITICAL_KEYWORDS = [
    ...
    '日米', 'トランプ',
    '首脳会談', 'れいわ', '幹事長', '政調会長', '日銀総裁',
    '裏金', '派閥', '旧統一教会', '倫理審査会', '解散', '支持率',
]

# 除外キーワード（※海外の首脳、トランプ氏に関係ない地域・紛争を追加しました）
EXCLUDE_KEYWORDS = [
    'プロレス', '新日本プロレス', 'サッカー', '野球', 'バスケ',
    '映画', 'ドラマ', 'アイドル', '鈴木みのる',
    'バイデン', 'ハリス', 'ゼレンスキー', 'マクロン', 'スターマー', 'プーチン', '習近平',
    'ウクライナ', 'ロシア', 'イスラエル', 'ガザ', 'パレスチナ', 'ハマス', '中東', 'EU', '欧州',
    '国連', 'テロ', '反政府デモ'
]
...
def check_political_relevance(title, description):
    ...
    try:
        # ※プロンプトを国内とトランプ氏特化に書き換えました
        prompt = f"""
以下のニュースが「日本の国内政治」または「ドナルド・トランプ氏の動向」にどれだけ関連しているか、0〜100点で評価してください。
【重要】日本国内とトランプ氏以外の海外ニュース（他国の選挙、他国間の紛争など）は、政治ニュースであっても一律0点としてください。
数字のみで回答してください。
...
def generate_ai_comment(title, description):
    ...
    try:
        ...
        prompt = f"""
以下のニュースを分析し、この出来事が今後の日本や世界にどのような影響を及ぼすか正確に予測してください。
【重要】「ご提示いただいたニュースに基づき〜」などの前置き、挨拶、まとめの言葉は一切出力しないでください。いきなり以下の指定フォーマットから書き始めてください。
